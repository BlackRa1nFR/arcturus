        ...
        try
        {
            synchronized (Emulator.getGameEnvironment().getAchievementManager().getAchievements())
            {
                ...
                for (Achievement achievement : Emulator.getGameEnvironment().getAchievementManager().getAchievements().values())
                {
                    ...
                    this.response.appendInt(nextLevel != null ? nextLevel.level : currentLevel.level); //
                    this.response.appendString("ACH_" + achievement.name + (nextLevel != null ? nextLevel.level : currentLevel.level)); //Target badge code
                    ...
                    this.response.appendInt(AchievementManager.hasAchieved(habbo, achievement) ? 1 : 0); //1 = Progressbar visible if the achievement is completed
                }
